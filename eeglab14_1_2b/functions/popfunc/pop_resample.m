...
function [ EEG ] = pop_resample( EEG, freq, fc, df) 
...
% -------------------------
if length(EEG) > 1
    [ EEG ] = eeg_eval( 'pop_resample', EEG, 'warning', 'on', 'params', { freq } );
    return;
end
...
%     fprintf('%d ', index1);	
    ...
    if index1 == 1
        ...
        if size(tmpres,1) == 1 
            EEG.pnts  = size(tmpres,2);
        else
            EEG.pnts  = size(tmpres,1);
        end
...
if isfield(EEG.event, 'latency')
    ...
    if EEG.trials > 1 % Epoched data; not recommended
        ...
        error( 'Resampling of epoched data is not recommended (due to anti-aliasing filtering)! Note: For epoched datasets recomputing urevent latencies is not supported. The urevent structure will be cleared.' )
...
%         figure; freqz(b, 1, 2^14, EEG.srate) % Debugging only! Sampling rate hardcoded as it is unknown in this context. Manually adjust for debugging!
        ...
        error('No Signal Processing toolbox! Need resample.m!')
